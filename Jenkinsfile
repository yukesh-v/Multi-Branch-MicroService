    ...
    stages {
        ...
        stage('Trivy fs Scan') {
                ...
                steps {
                    script {
                            withDockerRegistry(credentialsId: 'docker-cred') {
                                sh "docker build -t yukesh24/paymentservice:${GIT_COMMIT_REV} ."
                            }
                    }
                }
            }
            stage('Trivy Image Scan') {
                steps {
                    sh "trivy image --format table -o paymentservice-image-report.html yukesh24/paymentservice:${GIT_COMMIT_REV}"
                }
            }
            stage('Docker Push') {
                steps {
                    script {
                            withDockerRegistry(credentialsId: 'docker-cred') {
                                sh "docker push yukesh24/paymentservice:${GIT_COMMIT_REV}"
                            }
                        }
                }
            }
